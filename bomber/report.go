...
func newReport(size int, results chan *Result, output string) *report {
	return &report{
		statusStepDist: make(map[int]int),
	...
	for {
		...
		case res := <-r.results:
			if res.err != nil {
				...
				r.statusStepDist[res.statusStep]++
...
// Prints status step distribution.
func (r *report) printStatusSteps() {
	fmt.Printf("\nStatus step distribution:\n")
	for step, num := range r.statusStepDist {
